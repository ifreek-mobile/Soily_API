...
Request 例:
```json
{"latitude":"35.6895","longitude":"139.6917","direction":"南向き","location":"ベランダ"}
```
Validation:
...
| 種類        | 実行方法                    | 目的                                | 例 |
| ----------- | --------------------------- | ----------------------------------- | -- |
| unit (少)   | `pytest -m unit`            | 純粋関数/軽量検証 (最小)            | text処理 (将来) |
| integration | `pytest -m integration`     | FastAPI 内部ルータ/再試行/検証      | chat/trivia 異常系 |
| e2e         | `pytest -m e2e`             | サーバ実起動 + 契約(HTTP) 黒箱確認  | OpenAPI, CORS |
| smoke負荷   | `pytest -m e2e -k smoke`    | 軽並行で 5xx 無し/429 制御確認      | smoke_parallel |

主なカバレッジ:
- /chat: JSON不正→502, 500/429 リトライ成功/失敗, 全タイムアウト→503, 300文字トリム, 環境変数上書き
- /trivia: 天気フェーズ例外フォールバック, 20文字制約, バリデーション境界値, 空応答503
